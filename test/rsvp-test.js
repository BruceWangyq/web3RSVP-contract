...
describe("Create New Event", function () {
  it("should create a new event, rsvp, and confirm", async function () {
    ...
    await expect(
      rsvpContract.createNewEvent(timestamp, deposit, maxCapacity, eventDataCID)
    ).to.emit(rsvpContract, "NewEventCreated");

    await expect(
      rsvpContract.createNewRSVP(eventID, { value: deposit })
    ).to.emit(rsvpContract, "NewRSVP");

    await expect(
      rsvpContract.connect(address1).createNewRSVP(eventID, { value: deposit })
    ).to.emit(rsvpContract, "NewRSVP");
